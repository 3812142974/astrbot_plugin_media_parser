...
3. Cookie 失效时，插件向管理员私聊发送提示，告知可用指令
4. 管理员在**与管理员的私聊**中发送官方指令 `bilibili登录`（别名 `bili登录` / `b站登录` / `bilibili_cookie`）发起登录
5. 插件会同时发送**扫码二维码**与**登录链接**，二选一即可：用哔哩哔哩客户端扫二维码，或在浏览器/客户端打开链接授权；完成后 Cookie 自动更新，无需手动替换

管理员也可在私聊中随时直接发送 `bilibili登录` 主动发起登录，不受自动请求冷却限制。这些指令由 AstrBot 原生命令系统接管，**只会触发对应流程、不会转交给 LLM**，且仅限管理员（AstrBot 主配置中设置的管理员）使用：

- **非管理员**发送指令会收到「该指令仅限管理员使用，你不是管理员。」提示，指令被忽略。
- **Cookie 增强未开启**（`B站增强 → 携带Cookie解析` 关闭）或**管理员协助登录未开启**（`B站增强 → 管理员协助登录 → 启用` 关闭）时，管理员发送 `bilibili登录` 会收到对应的「未开启」提示。这两项默认均为关闭。
- `清理媒体` 指令同样仅限管理员。

> **参数说明**：`回复超时` 控制登录二维码轮询的最长时间（默认 1440 分钟）；`请求冷却` 只控制自动检测触发的协助请求。B站二维码有效期由服务端控制，插件会在约 3 分钟内结束轮询。

...
4. 管理员在**与管理员的私聊**中发送官方指令 `bili登录`（别名 `b站登录` / `bili_cookie`）发起扫码登录
5. 扫码完成后 Cookie 自动更新，无需手动替换

管理员也可在私聊中随时直接发送 `bili登录` 主动发起扫码更新，不受自动请求冷却限制。该指令由 AstrBot 原生命令系统接管，**只会触发协助登录流程、不会转交给 LLM**，且仅接受管理员私聊。

...
管理员也可在私聊中随时直接发送 `bilibili登录` 主动发起登录，不受自动请求冷却限制。该指令由 AstrBot 原生命令系统接管，**只会触发协助登录流程、不会转交给 LLM**；并通过 AstrBot 官方 `PermissionType.ADMIN` 权限限制为**仅管理员可用**（需在 AstrBot 主配置中设置管理员），非管理员发送会直接被拦截。`清理媒体` 指令同样仅限管理员。
